...
/*   Updated: 2019/04/09 20:07:01 by seshevch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "Form.hpp"

Form::Form() : _name("ANON"), _grade_sign(1), _grade_exe(1), _target("non")
{
	_sign = 0;
	return ;
}
Form::Form(std::string name, int const grade_sign, int const grade_exe, std::string target) : _name(name), _grade_sign(grade_sign), _grade_exe(grade_exe), _target(target)
{
...
Form::Form(Form const & src) : _name(src._name), _grade_sign(src._grade_sign), _grade_exe(src._grade_exe) , _target(src._target)
{
	*this = src;
}
...
{
	if (executor.getGrade() > _grade_exe || !_sign)
		throw GradeTooLowException();
}